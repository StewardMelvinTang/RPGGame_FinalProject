...
void Enemy::Hit(float damage, std::string turretName) {
	hp -= damage;
	if (hp <= 0) {
		if (turretName != "GlueGunner") OnExplode();
		...
		getPlayScene()->AddScore(money * 5);
		getPlayScene()->enemyKilled++;
		getPlayScene()->EnemyGroup->RemoveObject(objectIterator);
		AudioHelper::PlayAudio("explosion.wav");
	}

	if (turretName == "GlueGunner"){
		// * handle slow effect here
		cout << "Slow Effect Activated\n";
		speedMultiplier = 0.25f;
		glueSlowCooldown = 3.5f;
	}
}
...
void Enemy::Update(float deltaTime) {
	...
	if (glueSlowCooldown > 0.0f){
		glueSlowCooldown -= 1.0f * deltaTime;
		// cout << "Cooldown : " << glueSlowCooldown << endl;
	}
	if (glueSlowCooldown <= 0.0f){
		speedMultiplier = 1.0f;
	}
	VirtualUpdate(deltaTime);
